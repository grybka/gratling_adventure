...
from world.LocationMap import LocationMap
...
class GameEngine(AbstractEngine):
    def __init__(self,display:AbstractDisplay):
        ...
        self.world_map=LocationMap()
        #self.assign_object_location(self.player_object,location_1)
        self.assign_object_location(self.player_object,self.world_map.rooms[0][0])
    ...
    def assign_object_location(self,object:GameObject,location:GameLocation):
        ...
        if object==self.player_object:
            self.display.update_map(self.world_map.get_map_image(location))     
            ...
            self.writer.announce_action("A "+object.get_choice_word()+" appears in a flash of logic\n")
        else:
